...
func basicPipeline() async throws {
    ...
    guard let imageURL = Bundle.module.url(forResource: "test-icon", withExtension: "png"),
        ...
        print("❌ Failed to load test-icon.png from resources")
        return
    }
    #else
    guard let imageURL = Bundle.module.url(forResource: "test-icon", withExtension: "png"),
    ...
    if let clusterCount: Int = result.metadata(for: "clusterCount") {
        ...
        for i in 0..<clusterCount {
            ...
            guard let layerBuffer = result.buffer(named: "layer_\(i)") else {
                print("  ⚠️  Warning: layer_\(i) buffer not found")
                continue
            }
            ...
            guard let layerImage = createLayerImage(from: layerBuffer, width: width, height: height) else {
                print("  ⚠️  Warning: Failed to create image from layer_\(i) buffer")
                continue
            }
            ...
            if let tiffData = layerImage.tiffRepresentation,
               ...
               let pngData = bitmapImage.representation(using: .png, properties: [:]) {
                ...
                print("  ✅ Saved: layer_\(i).png (\(sizeKB)KB buffer → \(pngData.count/1024)KB PNG)")
            }
            #else
            // iOS placeholder
            let filePath = "\(outputDir)/layer_\(i).png"
        ...
        📋 Configuration:
        • Source image: test-icon.png (1024x1024)
        ...
        • RGB → LAB color conversion with configurable axis scaling
        • SLIC superpixel segmentation (13 Metal kernels)
        • K-means++ clustering with smart initialization (11 Metal kernels)
        ...
        • Icon design iteration (recolor individual elements)
        • Asset preparation (separate layers for different themes)
        • Color analysis (understand dominant colors)
...
func reusableTemplate() async throws {
    ...
    guard let imageURL = Bundle.module.url(forResource: "test-icon", withExtension: "png"),
          let image = NSImage(contentsOf: imageURL) else {
        print("❌ Failed to load test-icon.png")
        return
    }
    #else
    guard let imageURL = Bundle.module.url(forResource: "test-icon", withExtension: "png"),
